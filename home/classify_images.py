...
def classify_images(images_dir, results_dic, model):
    ...
    filenames = [f for f in listdir(str(images_dir)) if not f.startswith('.')] 
    pet_labels = []
    for key in results_dic:
        pet_labels.append(results_dic[key][0])
    
    
    classifier_label_name = []
    for i in filenames:
        classifier_result= classifier(str(images_dir)+i, model)
        classifier_label_name.append(classifier_result)
    
    
    classifier_labels=[]
# Sets string to lower case letters
    for a in classifier_label_name:
        class_name = a.lower()


# Strip off starting/trailing whitespace characters 
        class_name = class_name.strip()
        classifier_labels.append(class_name)
